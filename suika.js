(function () {
  ...
  var FRUIT_NAMES = ["Leaf","Bud","Joint","Pills","Vial","Ice","Brick","Cash Stack","Diamond","Empire","Narco King"];

  var GRAVITY = 2200;
  var RESTITUTION = 0.2;
  var FRICTION = 0.35;
  var CONTACT_FRICTION = 0.97;
  var BEST_KEY = "narcos_best";
  var GAMES_KEY = "narcos_games";
  var DEATH_Y = 40;
  var REST_MS = 1000;
  ...
  var overlay = document.getElementById("suikaOverlay");
  var finalScoreEl = document.getElementById("suikaFinal");
  var rewardEl = document.getElementById("suikaReward");
  ...
  function mergeFruit(i, j) {
    ...
    chain++;
    totalMerges++;
    if (nt > bestTier) bestTier = nt;
    if (nt === FRUITS.length - 1) watermelons++;
    fruits.push({
      t: nt,
      x: mx,
      y: my,
  ...
  function endGame() {
    ...
    if (newBestEl) newBestEl.style.display = isNew ? "" : "none";
    if (window.suikaIdleFinish) window.suikaIdleFinish(score, { merges: totalMerges, watermelons: watermelons, bestTier: bestTier });
    if (rewardEl) rewardEl.textContent = window.suikaIdleRewardText || '';
    if (statsEl) {
      statsEl.innerHTML = '<span class="chip">🧊 merges: ' + totalMerges + '</span>' +
        '<span class="chip">🎯 best: ' + FRUIT_NAMES[bestTier] + '</span>' +
        '<span class="chip">👑 kings: ' + watermelons + '</span>';
    }
  ...
  function drawFruitShape(t, R) {
    var palette = [
      ["#4caf50", "#a5d6a7", "#1b5e20"], // leaf
      ["#2e7d32", "#81c784", "#0d3b12"], // bud
      ["#c4a574", "#e8d5b5", "#6d4c2b"], // joint
      ["#e91e63", "#f48fb1", "#880e4f"], // pills
      ["#26c6da", "#b2ebf2", "#006064"], // vial
      ["#b3e5fc", "#e1f5fe", "#0277bd"], // ice
      ["#d4a574", "#f0d9b5", "#6d4c2b"], // brick
      ["#43a047", "#a5d6a7", "#1b5e20"], // cash
      ["#42a5f5", "#bbdefb", "#0d47a1"], // diamond
      ["#ffd54f", "#fff8e1", "#f57f17"], // empire
      ["#ab47bc", "#e1bee7", "#4a148c"]  // narco king
    ];
  ...
  function drawBackground() {
    ...
    ctx.fillText("HEAT LINE", W / 2, TOP_Y + 4);
  }
