...
const Leaderboard = (()=>{
  ...
  function getMyId(){
    if(!myId){
      let pid=null;
      try{ pid=localStorage.getItem('narcos_pid'); }catch(e){}
      myId=pid||G.save.playerId||('p'+Math.random().toString(36).slice(2,10));
      try{ localStorage.setItem('narcos_pid',myId); }catch(e){}
  ...
  function mySkinEmoji(){
    ...
    return {classic:'💵',golden:'🥇',crystal:'💎',lava:'🌋',galaxy:'🌌',void:'⚫',mint:'🍃',royal:'👑',asc:'⭐',starbound:'✨',cosmic:'🕳️',berry:'🍓',peach:'🍑',candy:'🍬',toxic:'☢️',frost:'❄️',inferno:'🔥'}[id]||'💵';
  }
