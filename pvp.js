'use strict';
/* ============================================================
   NARCOS EMPIRE — Cartel Raids
   Every idle operation slowly fills your guarded
   STASH in the cloud. Raid other cartels to steal product from
...
const PVP=(()=>{
  ...
  function myName(){ return (G.save&&G.save.name)||'Cartel'; }
  function defense(){
    const base=Math.floor((1+G.save.level*0.1)*(1+G.save.ascensions*0.2)*(1+shopLvl('sh13')*0.15)*10);
    const army=(typeof cartelDefense==='function')?cartelDefense():0;
    return Math.floor(base+army);
  }
  function power(){
    const base=Math.floor((1+G.save.level*0.06)*(1+G.save.ascensions*0.15)*(1+shopLvl('sh12')*0.25)*(1+(G.save.bossKills||0)*0.02)*10);
    const army=(typeof cartelPower==='function')?cartelPower():0;
    return Math.floor(base+army);
  }
  ...
  function targetList(){
    ...
    if(doc&&doc.p){
      for(const k in doc.p){
        if(k!==myId()&&doc.p[k]) out.push({id:k,name:doc.p[k].name||'Cartel',stash:doc.p[k].s||0,def:doc.p[k].d||5});
      }
    }
    if(!out.length&&typeof Leaderboard!=='undefined'&&Leaderboard.getScores){
      const sc=Leaderboard.getScores();
      sc.forEach(s=>{ if(s.id!==myId()) out.push({id:s.id,name:s.name||'Cartel',stash:0,def:5+(s.level||1)*2}); });
    }
    return out;
  }
  ...
  async function raid(){
    ...
    if(!list.length){
      lastResult='No other cartels found to raid yet!';
      raiding=false;
      render();
    ...
    if(win){
      ...
      if(steal<=0){
        ...
        lastResult='Raid on '+escapeHtml(tgt.name)+' SUCCEEDED! Stole +'+(typeof fmtW==='function'?fmtW(steal):fmt(steal))+'!';
      }
      ...
      st.lost=(st.lost||0)+1;
      lastResult='Raid on '+escapeHtml(tgt.name)+' FAILED — you lost '+(typeof fmtW==='function'?fmtW(penalty):fmt(penalty))+'.';
      toast(lastResult,'💥');
      beepRaid(false);
    }
  ...
  function render(){
    ...
    if(rs){
      ...
      rs.innerHTML=(lastResult?escapeHtml(lastResult)+'<br>':'')+
        ...
        ' · Raids won: '+(st.won||0)+' · Stolen: '+(typeof fmtW==='function'?fmtW(st.stolen||0):fmt(st.stolen||0));
    }
    const b=el('raidBtn');
