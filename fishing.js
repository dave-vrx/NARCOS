'use strict';
/* ============================================================
   NARCOS EMPIRE — Smuggling mini game
...
const Fishing=(()=>{
  const ZONES=[
    {n:'Shallows',e:'🌿'},{n:'River Delta',e:'🏞️'},{n:'Coastal',e:'🪷'},
    {n:'Open Sea',e:'🌊'},{n:'Deep',e:'🌌'},{n:'Forbidden',e:'🪐'}
  ];
  const SPECIES=[
  ...
  const RODS=[
    {id:'r0',n:'Raft',e:'🛶',cost:0,     zone:0,speed:0.9, sweet:0.15,mult:1},
    {id:'r1',n:'Rowboat',e:'🚣',cost:2500,    zone:1,speed:0.84,sweet:0.17,mult:1.4},
    {id:'r2',n:'Speedboat',e:'🚤',cost:20000,   zone:2,speed:0.78,sweet:0.19,mult:1.9},
    {id:'r3',n:'Cigarette Boat',e:'🏁',cost:150000,  zone:3,speed:0.72,sweet:0.21,mult:2.6},
    {id:'r4',n:'Smuggler Trawler',e:'⛵',cost:1.2e6,    zone:4,speed:0.66,sweet:0.23,mult:3.5},
    {id:'r5',n:'Submarine',e:'🛥️',cost:10e6,   zone:5,speed:0.60,sweet:0.25,mult:4.7},
    {id:'r6',n:'War Sub',e:'🚢',cost:80e6,      zone:6,speed:0.53,sweet:0.28,mult:6.3},
    {id:'r7',n:'Ghost Ship',e:'👻',cost:6e8,  zone:7,speed:0.45,sweet:0.32,mult:8.5}
  ];
  ...
  function cast(){
    ...
    state='casting';
    setStatus('Sending drop… 📦');
    const b=el('fishCastBtn'); if(b){ b.disabled=true; b.textContent='Sending…'; }
  ...
  function bite(){
    ...
    setStatus('Contact! Tap the bar to PULL IN! ⚡');
    sweetW=rod().sweet;
  ...
  function resolveCatch(inSweet,near,timeout){
    ...
    const fail=()=>{
      state='idle';
      setStatus('The drop got away! 💨 Send again.');
      el('fishCastBtn').disabled=false; el('fishCastBtn').textContent='📦 Send Drop';
      beepTone();
      render();
    };
  ...
  function recordCatch(fish,perfect){
    ...
    toast('Hauled: '+fish.name+' ('+zoneName(fish.zone)+') +'+(typeof fmtW==='function'?fmtW(m):fmt(m))+(cry?' +'+cry+' 💎':'')+'!',fish.emoji);
    state='idle';
    el('fishCastBtn').disabled=false; el('fishCastBtn').textContent='📦 Send Drop';
    beepCatch();
  ...
  function showCatchModal(fish,m,cry){
    ...
    el('fishCatchReward').textContent='+'+(typeof fmtW==='function'?fmtW(m):fmt(m))+(cry?' · +'+cry+' 💎':'');
  ...
  function upgradeRod(){
    ...
    if(!next){ toast('You own the best boat already!','♾️'); return; }
    if(G.save.melons<next.cost){ toast('Need '+(typeof fmtW==='function'?fmtW(next.cost):fmt(next.cost))+' for the '+next.n+'!','💸'); return; }
  ...
  function renderIndex(){
    ...
    el('fishIndexCount').textContent=caught+' / '+buildCatalog().length+' hauls discovered';
    let h='';
  ...
  function render(){
    ...
    const nxt=el('fishRodNext'); if(nxt) nxt.textContent=(RODS[(fs.rod||0)+1]?('Next: '+RODS[(fs.rod||0)+1].n+' — '+(typeof fmtW==='function'?fmtW(RODS[(fs.rod||0)+1].cost):fmt(RODS[(fs.rod||0)+1].cost))):'MAX boat!');
    const up=el('fishUpgradeBtn');
    if(up){ up.disabled=(fs.rod||0)>=RODS.length-1||G.save.melons<(RODS[(fs.rod||0)+1]||{}).cost; }
    const zn=el('fishZoneName'); if(zn) zn.textContent='Route: '+ZONES[0].n+' → '+zoneName(maxZone());
    const st=el('fishStats'); if(st) st.textContent='📦 '+fmt(fs.caught||0)+' hauls · '+Object.keys(fs.index||{}).length+'/'+buildCatalog().length+' discovered'+(fs.royal?(' · 👑 '+fs.royal):'');
    const b=el('fishCastBtn'); if(b&&state==='idle'){ b.disabled=false; b.textContent='📦 Send Drop'; }
